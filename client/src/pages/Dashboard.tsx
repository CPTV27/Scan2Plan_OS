...
import { Users, FolderKanban, TrendingUp, AlertTriangle, Target, Trophy, DollarSign, TrendingDown, Mail, Clock, ChevronRight } from "lucide-react";
...
export default function Dashboard() {
  ...
  // Find biggest open opportunity
  const biggestOpportunity = leads
        ...
        <main className="flex-1 p-4 md:p-8 overflow-auto">
          <header className="mb-8">
            <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
              <div>
                <h2 className="text-3xl font-display font-bold">{greeting}, {firstName}</h2>
                <p className="text-muted-foreground mt-1">
                  {format(new Date(), "EEEE, MMMM d")} • Here's your business at a glance
                </p>
              </div>
              <AIAssistant />
            </div>
            ...
            {/* Key Action Banner */}
            {biggestOpportunity && Number(biggestOpportunity.value) > 0 && (
              <Link href={`/deals/${biggestOpportunity.id}`}>
                <div className="flex items-center justify-between p-4 rounded-lg bg-primary/10 border border-primary/20 hover:bg-primary/15 transition-colors cursor-pointer">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold">{biggestOpportunity.clientName}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <p className="text-2xl font-bold text-primary">${Number(biggestOpportunity.value).toLocaleString()}</p>
                      <Badge variant="secondary">{biggestOpportunity.dealStage}</Badge>
                    </div>
...
function StatCard({ title, value, icon: Icon, trend, color, highlight }: {
  ...
  return (
    <Card className={`border-border shadow-sm hover:shadow-md transition-all duration-200 ${highlight ? 'ring-2 ring-primary/30 bg-primary/5' : ''
      }`}>
        ...
        <div className="space-y-3">
          {proposalLeads.map((lead) => (
            <div
              ...
              className={`flex items-center justify-between gap-4 p-3 rounded-lg ${lead.status === 'stale'
