...
export function registerInvoiceRoutes(app: Express): void {
  ...
  app.get("/api/projects/:id/invoices", isAuthenticated, requireRole("ceo"), async (req, res) => {
    try {
  ...
  app.get("/api/invoices/lead/:leadId", isAuthenticated, requireRole("ceo"), async (req, res) => {
    try {
